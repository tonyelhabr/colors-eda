...
```{r viz_colors_nas_bylg, echo = FALSE}
...
Of course, a good visualization is always appreciated.
...
colors_named_ig
igraph::V(colors_named_ig)$node_label <- names(igraph::V(colors_named_ig))
igraph::V(colors_named_ig)$node_size <- igraph::degree(colors_named_ig)

viz_colors_named <-
  colors_named_ig %>%
  ggraph::ggraph(layout = "linear", circular = TRUE) +
  ggraph::geom_edge_arc() +
  # ggraph::geom_edge_arc(edge_width = 0.125, aes(alpha = ..index..)) +
...
Moreover, given the "set" nature of the data set, I think that
