...
Nevertheless, I can use [Alex Bresler](http://asbcllc.com/)'s 
...
I think that this circular diagram does a good job of
...
I can also use a heat map to visualize the distance values.
...
```{r colors_ord2_nba_rgb_dist_viz_prank, echo = FALSE}
...
Is the [RGB model](https://en.wikipedia.org/wiki/RGB_color_model) 
...
```{r colors_ord2_nba_rgb_hsl_yuv_dist, echo = FALSE, fig.height = 10, fig.width = 6}
cor_dist_data <-
  colors_ord2_nba_rgb_tidy %>% 
  do_pairwise_dist("euclidean") %>% 
