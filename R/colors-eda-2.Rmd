...
create_kable <- function(data, num_show = 20) {
  ...
  if(show_fn) {
    out <-
      out %>%
      kableExtra::add_footnote(c(sprintf("# of total row: %.0f", num_rows)), notation = "number")
  }
  out
}
```
...
```{r viz_colors_nba_ord2, echo = FALSE}
colors_nba_ord2 <-
  colors_nba_tidy_ord2 %>% 
...
viz_colors_nba_ord2 <-
  colors_nba_ord2  %>% 
  mutate(slug_name = paste0(slug, ": ", name)) %>% 
  # ggplot(aes(x = name, color = name, fill = name)) + 
  ggplot(aes(x = slug_name, color = primary, fill = secondary)) + 
  geom_bar(size = 1.5) +
  facet_wrap(~toupper(league), scales = "free_y") +
  coord_flip() +
  scale_x_discrete(NULL) + 
  scale_y_continuous(NULL) + 
  # scale_fill_manual(values = colors_nba_ord2$primary) + 
  # scale_color_manual(values = colors_nba_ord2$secondary) + 
  viz_theme_base +
...
```{r colors_nba_ord2_rgb_tidy, results = "markdown"}
colors_nba_ord2_rgb_tidy <-
  colors_nba_tidy_ord2 %>%
  add_rgb_cols() %>% 
  select(-hex) %>%
...
```{r colors_nba_ord2_rgb_dist, results = "markdown"}
...
```{r colors_nba_ord2_hsl_yuv_dist, results = "markdown"}
colors_nba_ord2_hsl_tidy <-
  colors_nba_tidy_ord2 %>%
  add_hsl_cols() %>% 
  select(-hex) %>%
  tidyr::gather(rgb, value, h, s, l)

colors_nba_ord2_yuv_tidy <-
  colors_nba_tidy_ord2 %>%
  add_yuv_cols() %>% 
  select(-hex) %>%
...
```{r colors_nba_ord2_rgb_hsl_yuv_dist, echo = FALSE}
cor_dist_data <-
  colors_nba_ord2_rgb_tidy %>% 
  do_pairwise_dist("euclidean") %>% 
...
```{r colors_nba_ord2_rgb_sim, echo = FALSE}
...
```{r colors_nba_ord2_h2h}
colors_nba_ord2_h2h <-
  colors_nba_ord2_rgb_tidy %>% 
  do_pairwise_dist("euclidean")
```

```{r colors_nba_ord2_rgb_dist_debug, echo = FALSE, echo = FALSE}
colors_nba_ord2_h2h %>% 
  group_by(ord, name1) %>% 
  do(slice(., c(1, n()))) %>% 
  ungroup()
...
```{r viz_colors_nba_ord2_h2h_byord_setup, echo = FALSE}
add_nm_and_lg_cols <- function(data) {
  data %>%
  left_join(nms_nba %>% select(name1 = name, league1 = league), by = c("name1")) %>%
  left_join(nms_nba %>% select(name2 = name, league2 = league), by = c("name2"))
}

colors_nba_ord2_h2h_ranks <-
  colors_nba_ord2_h2h %>%
  add_nm_and_lg_cols() %>% 
  group_by(ord, name1) %>%
...
nms_nba_ranks <-
  colors_nba_ord2_h2h_ranks %>% 
  left_join(
    nms_nba %>% select(name1 = name, slug1 = slug, league),
    by = c("name1")
  )
...
```{r colors_nba_ord2_h2h_byord_viz, echo = FALSE}
create_colors_nba_ord2_h2h_viz <- function(data, ranks) {
  data %>% 
    ...
    # mutate_at(vars(name1, name2), funs(factor(., levels = lvls_nba_ranks)))
    mutate(name1 = reorder(name1, rank1),
           name2 = reorder(name2, rank2))
}
colors_nba_ord2_h2h_byord_viz <-
  colors_nba_ord2_h2h  %>%
  create_colors_nba_ord2_h2h_viz(colors_nba_ord2_h2h_ranks)
```


```{r colors_nba_ord2_h2h_byord_viz_debug, include = FALSE, eval = FALSE}
# colors_nba_ord2_h2h_viz %>% arrange(rank1)
# colors_nba_ord2_h2h_viz %>% arrange(name1)
# levels(colors_nba_ord2_h2h_viz$name1)
# levels(colors_nba_ord2_h2h_viz$name2)
```

```{r viz_colors_nba_ord2_h2h_setup, echo = FALSE}
lab_subtitle_nba <-
...
viz_labs_colors_nba_ord2_h2h <-
  labs(
    title = "Pairwise Distance of Colors",
    subtitle = lab_subtitle_nba,
    caption = lab_caption_nba,
    x = NULL,
    y = NULL
  )

visualize_colors_nba_ord2_h2h_const <-
  function(data, value = "value", emphasize_top = TRUE, pct_top = 0.02) {
    ...
    viz <-
      ...
      scale_x_discrete(labels = labs_nba_ranks) +
      scale_y_discrete(labels = labs_nba_ranks) +
...
```{r viz_colors_nba_ord2_h2h_byord, include = FALSE, eval = FALSE}
viz_colors_nba_ord2_h2h_byord <-
  colors_nba_ord2_h2h_byord_viz %>%
  visualize_colors_nba_ord2_h2h_const() +
...
```{r colors_nba_ord2_h2h_viz, echo = FALSE}
colors_nba_ord2_h2h_viz <-
  colors_nba_ord2_h2h %>%
...
```{r viz_colors_nba_ord2_h2h, echo = FALSE}
viz_colors_nba_ord2_h2h <-
  colors_nba_ord2_h2h_viz %>%
  visualize_colors_nba_ord2_h2h_const()
viz_colors_nba_ord2_h2h
```
...
```{r colors_nba_ord2_h2h_viz_prank, echo = FALSE, results = "markdown"}
colors_nba_ord2_h2h_viz_prank <-
  colors_nba_ord2_h2h_viz %>% 
